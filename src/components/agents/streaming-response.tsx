...
export interface StreamingResponseProps {
  ...
  showActions?: boolean;
  /** Replaces the built-in markdown utility classes on the content wrapper
   *  instead of appending to them (vendored extension) — pass e.g. the app's
   *  `.text-block` when the consumer owns typography. */
  contentClass?: string;
  /** Localized label for the built-in copy action (vendored extension). */
  copyLabel?: string;
  /** Localized label for the copied state of the copy action. */
  copiedLabel?: string;
  /** Hides the built-in helpful/not-helpful feedback pair (vendored extension). */
  showFeedback?: boolean;
  /** Extra actions rendered after the built-in copy/retry pair (vendored
   *  extension) — e.g. a fork button. */
  actions?: ReactNode;
  className?: string;
        ...
        {shouldShowActions ? (
          ...
          >
            <div className={cn("flex items-center gap-0.5", actionsClassName)}>
              {canCopy ? (
                <ResponseAction
                  label={copied ? (copiedLabel ?? "Copied") : (copyLabel ?? "Copy response")}
                  onClick={handleCopy}
                >
                  {copied ? (
